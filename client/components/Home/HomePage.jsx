...
const Home = ({ providerId, custumorId }) => {
  ...
  useEffect(() => {
    if (searchTerm.trim() !== '') {
      ...
      axios
        .get(
          `http://192.168.104.4:3000/provider/search?category=${searchTerm}`
        )
        .then((response) => {
      ...
      if (providerId) {
        try {
          const response = await axios.get(
            `http://192.168.104.4:3000/provider/getOne/${providerId}`
          );
  ...
  useEffect(() => {
    const fetchProfileUrl = async () => {
      if (custumorId) {
        try {
          const response = await axios.get(
            `http://192.168.104.4:3000/custumor/getOne/${custumorId}`
          );
          const imgprof = response.data.imgprof;
          console.log('imgprof taswirraaaa:', imgprof); // Check the value of imgprof
